...
public:
	...
	void setInputTarget(const pcl::PointCloud<pcl::PointNormal>::Ptr& cloud_target,
		const std::shared_ptr<pcl::KdTreeFLANN<pcl::PointNormal>> kd_tree_ptr = nullptr) {
	...
	virtual void align(pcl::PointCloud<pcl::PointNormal>& cloud_aligned,
		...
		tEnd = clock();
		std::cout << "Number of corres-pairs from fpfh matching:" << correspondences.size() << std::endl;
		...
		params.rotation_max_iterations = 1000; // 100
		params.rotation_gnc_factor = 1.4;
		params.rotation_estimation_algorithm =
			teaser::RobustRegistrationSolver::ROTATION_ESTIMATION_ALGORITHM::GNC_TLS;
		params.rotation_cost_threshold = 1e-16;// 0.005;
		...
		solver.solve(src_cloud, tgt_cloud, correspondences, m_nThreads);
		tEnd = clock();
		...
		printf("time_fpfh = %04dms, time_corres = %04dms, time_reg = %04dms, corresNum = %04d\n",
