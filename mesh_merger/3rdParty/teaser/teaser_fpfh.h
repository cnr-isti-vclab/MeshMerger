...
namespace gabor {
	...
	public:
		...
		std::shared_ptr<std::vector<int> > m_vkeypoint_indices;
		...
		void computeKeypoints() {
			...
			//m_cloud_keypoints = cloud_xyz;
//#if LOG
			std::cout << "data after voxel-sampling with sample radius:\n" << m_cloud_keypoints->points.size() << "," << m_dSampleRadius << std::endl;
//#endif // LOG
		...
		template<typename T>
		inline void cvt2Teaser_cloud(const std::shared_ptr<pcl::PointCloud<T>>& cloud_pcl,
