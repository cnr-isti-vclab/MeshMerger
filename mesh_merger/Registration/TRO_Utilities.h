...
Eigen::Matrix4f Rotate(float rotinDegree, int axis = 1);
Eigen::Matrix4f changeSymmetry(int axis = 0);
std::vector<float> computeViewPosDifference(const std::vector<Eigen::Vector3f>& devicePosOrient);
Eigen::Vector3f vectorFromAngle(float radAngle, int dir = 2);
...
template <typename T>
inline void randSampleInPlaceFun(std::shared_ptr<pcl::PointCloud<T>>& cloud_in,
	int nCnt) {
...
template <typename T>
inline void voxelSampleFun( const std::shared_ptr<pcl::PointCloud<T>>& cloud_in,
	std::shared_ptr<pcl::PointCloud<T>>& cloud_out,
	double dGridSize) {
	if (dGridSize < 0) {
		pcl::copyPointCloud(*cloud_in, *cloud_out); 
		return;
...
template <typename T>
inline void voxelSampleInPlaceFun(std::shared_ptr<pcl::PointCloud<T>>& cloud_in,
	double dGridSize) {
	std::shared_ptr<pcl::PointCloud<T>> cloud_out(new pcl::PointCloud<T>());
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_xyz)
{
	...
	pcl::copyPointCloud<pcl::PointNormal, pcl::Normal>(*cloud_normals, *cloud_nor);
#ifdef LOG
	//std::cout << "no. of normals after decoupling :\n" << cloud_nor->points.size() << std::endl;
	//std::cout << "no. of points after decoupling:\n" << cloud_nor->points.size() << std::endl;
#endif // LOG
...
inline void read_cloud(std::string& sFileName,
	std::shared_ptr< pcl::PointCloud<T>> & cloud_normal) {
...
public:
	...
	//// 
	template <typename T>
	void load(const std::shared_ptr<pcl::PointCloud<T>>& cloud_in) {
		clear();
...
private:
	...
	void delete_nan_element() {
		pcl::Normal norm_pt;
		std::shared_ptr<std::vector<int> > valid_indices(new std::vector<int>);
		...
		if (m_cloud_nor->points.size() != valid_indices->size()) {
			printf("Delete nan normals: %02d, %05d/%05d\n",
